...
JOBHUNT_REPO = os.environ.get("JOBHUNT_REPO", "AtmanTest/jobhunt")
JOBHUNT_BRANCH = os.environ.get("JOBHUNT_BRANCH", "main")
CLOSED_JOBS_URL = f"https://raw.githubusercontent.com/{JOBHUNT_REPO}/{JOBHUNT_BRANCH}/docs/closed_jobs.json"
CLOSED_JOBS_GITHUB_PATH = "docs/closed_jobs.json"

# Identité du candidat : fournie par l'environnement, jamais stockée dans le dépôt
APPLICANT_NAME = os.environ.get("JOBHUNT_APPLICANT_NAME", "")
APPLICANT_TITLE = os.environ.get("JOBHUNT_APPLICANT_TITLE", "Consultant QA")
APPLICANT_INTRO_FR = os.environ.get(
    "JOBHUNT_INTRO_FR",
    "Je mets ma pratique de la recette fonctionnelle — stratégie de test, matrices de couverture, "
    "automatisation des régressions et suivi des anomalies — au service de vos livraisons.")
APPLICANT_AVAILABILITY_FR = os.environ.get(
    "JOBHUNT_AVAILABILITY_FR", "Disponible pour un échange à votre convenance.")
APPLICANT_INTRO_EN = os.environ.get(
    "JOBHUNT_INTRO_EN",
    "I bring hands-on functional testing practice: test strategy, coverage matrices, regression "
    "automation and defect lifecycle management.")
...
@app.context_processor
def inject_cv_link():
    """The CV page is not served from the public repo — hide its links when the route is gone."""
    try:
        return dict()
    except Exception:
        return dict()
...
GITHUB_JOBS_URL = f"https://raw.githubusercontent.com/{JOBHUNT_REPO}/{JOBHUNT_BRANCH}/docs/jobs.json"
...
def _push_closed_jobs_to_github():
    ...
    try:
        ...
        sha_resp = requests.get(
            f"https://api.github.com/repos/{JOBHUNT_REPO}/contents/{CLOSED_JOBS_GITHUB_PATH}",
...
Compétences clés : {", ".join(matched_skills[:8]) if matched_skills else "QA, stratégie de test, automatisation, Agile / Scrum"}.

{APPLICANT_AVAILABILITY_FR}
        ...
        for line in r2.stdout.strip().split("\n"):
            if "|" in line:
                ...
                if name.startswith("v") or name.startswith("cv-"):
                    repo_tags.append({"name": name, "hash": h[:7]})
    except:
        pass

    # Fallback: GitHub API (works on Render where git clone is shallow)
    if not log_entries or not repo_tags:
        try:
            ...
            if r2.status_code == 200:
                for ref in r2.json():
                    ...
                    if name.startswith("v") or name.startswith("cv-"):
                        repo_tags.append({"name": name, "hash": ref["object"]["sha"][:7]})
        except:
            pass
