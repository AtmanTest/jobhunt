#!/usr/bin/env python3
"""
Uptime Monitor — SaaS de surveillance de sites web
...
def check_site(data, url):
    ...
    start = time.time()
    try:
        resp = requests.get(url, timeout=10, headers={"User-Agent": "Uptime-Monitor/2.0"})
    ...
    try:
        ...
        session = stripe.checkout.Session.create(
            ...
            line_items=[{
                "price_data": {
                    "currency": "eur",
                    "product_data": {"name": f"Uptime Monitor - {plan['name']}"},
